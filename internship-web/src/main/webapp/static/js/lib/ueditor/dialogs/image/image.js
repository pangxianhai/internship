...
//                }
            ...
            callback: function (editor, $w, url, state, title) {
                if (state == "SUCCESS") {
                    //显示图片计数+1
                    Upload.showCount++;
    ...
    var Upload = {
        ...
        config: function (sel) {
            ...
            var mid = me.editor.options.mid;

            url = url + (url.indexOf("?") == -1 ? "?" : "&")
                + "editorid=" + me.editor.id;//初始form提交地址;
        ...
        uploadComplete: function (r) {
            ...
            try {
                var json = eval('(' + r + ')');
                Base.callback(me.editor, me.dialog, json.url, json.state, json.title);
            } catch (e) {
                var lang = me.editor.getLang('image');
                Base.callback(me.editor, me.dialog, '', (lang && lang.uploadError) || 'Error!', json.title);
            }
        },
        submit: function (callback) {
            ...
            $(me.dialog).delegate(".edui-image-file", "change", function (e) {
                var filehz = $(this).val().split(".");

                if (!/(.gif|.jpg|.jpeg|.png)$/i.test("." + filehz[1])) {

                    $(".edui-modal-tip").html("别瞎搞,请上传格式(.gif|.jpg|.jpeg|.png)图片");
                    $(".edui-modal-tip").show();
                    setTimeout(function () {
                        $(".edui-modal-tip").hide();
                    }, 4000);
    ...
    UM.registerWidget('image', {
        ...
        "<table><tr><td><input class=\"edui-image-searchTxt\" type=\"text\">[请删除图片地址后缀: .jpg]</td>" +
